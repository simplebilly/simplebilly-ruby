=begin
#SimpleBilly API

#Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

The version of the OpenAPI document: 0.1.0
Contact: support@simplebilly.com
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'spec_helper'
require 'json'

# Unit tests for SimplebillyApi::CreditNoteApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'CreditNoteApi' do
  before do
    # run before each test
    @api_instance = SimplebillyApi::CreditNoteApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of CreditNoteApi' do
    it 'should create an instance of CreditNoteApi' do
      expect(@api_instance).to be_instance_of(SimplebillyApi::CreditNoteApi)
    end
  end

  # unit tests for create_credit_note
  # @param body 
  # @param [Hash] opts the optional parameters
  # @return [Invoice]
  describe 'create_credit_note test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for download_credit_note_pdf
  # @param credit_note_id 
  # @param [Hash] opts the optional parameters
  # @return [nil]
  describe 'download_credit_note_pdf test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for get_credit_note
  # @param credit_note_id 
  # @param [Hash] opts the optional parameters
  # @return [Invoice]
  describe 'get_credit_note test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for list_credit_notes
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page 
  # @option opts [Integer] :page_size 
  # @option opts [String] :search 
  # @option opts [Boolean] :include_deleted Soft-delete entities: set true to include rows with &#x60;deleted_at&#x60; set.
  # @return [Array<Invoice>]
  describe 'list_credit_notes test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
