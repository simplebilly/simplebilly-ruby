=begin
#SimpleBilly API

#Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

The version of the OpenAPI document: 0.1.0
Contact: support@simplebilly.com
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'spec_helper'
require 'json'

# Unit tests for SimplebillyApi::ProposeAssignmentsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'ProposeAssignmentsApi' do
  before do
    # run before each test
    @api_instance = SimplebillyApi::ProposeAssignmentsApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of ProposeAssignmentsApi' do
    it 'should create an instance of ProposeAssignmentsApi' do
      expect(@api_instance).to be_instance_of(SimplebillyApi::ProposeAssignmentsApi)
    end
  end

  # unit tests for propose_assignments_api
  # @param [Hash] opts the optional parameters
  # @option opts [Float] :min_confidence 
  # @option opts [String] :customer_id 
  # @return [Array<ProposedAssignment>]
  describe 'propose_assignments_api test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
