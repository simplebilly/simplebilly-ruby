=begin
#SimpleBilly API

#Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

The version of the OpenAPI document: 0.1.0
Contact: support@simplebilly.com
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'spec_helper'
require 'json'

# Unit tests for SimplebillyApi::BankingApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'BankingApi' do
  before do
    # run before each test
    @api_instance = SimplebillyApi::BankingApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of BankingApi' do
    it 'should create an instance of BankingApi' do
      expect(@api_instance).to be_instance_of(SimplebillyApi::BankingApi)
    end
  end

  # unit tests for bank_lookup_api
  # @param iban 
  # @param [Hash] opts the optional parameters
  # @return [BankLookup]
  describe 'bank_lookup_api test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for bank_transactions_api
  # @param [Hash] opts the optional parameters
  # @return [nil]
  describe 'bank_transactions_api test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for hebesatz_lookup_api
  # @param [Hash] opts the optional parameters
  # @option opts [String] :gemeindeschluessel 
  # @option opts [String] :plz 
  # @option opts [String] :name 
  # @option opts [String] :stichtag Stichtag for validity (YYYY-MM-DD); defaults to today. Picks row where valid_from &lt;&#x3D; date &lt;&#x3D; valid_to.
  # @option opts [String] :country_code 
  # @return [Array<HebesatzLookup>]
  describe 'hebesatz_lookup_api test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
