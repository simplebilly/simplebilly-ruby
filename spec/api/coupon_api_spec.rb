=begin
#SimpleBilly API

#Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.

The version of the OpenAPI document: 0.1.0
Contact: support@simplebilly.com
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'spec_helper'
require 'json'

# Unit tests for SimplebillyApi::CouponApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'CouponApi' do
  before do
    # run before each test
    @api_instance = SimplebillyApi::CouponApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of CouponApi' do
    it 'should create an instance of CouponApi' do
      expect(@api_instance).to be_instance_of(SimplebillyApi::CouponApi)
    end
  end

  # unit tests for coupon_restore
  # @param coupon_id 
  # @param [Hash] opts the optional parameters
  # @return [Coupon]
  describe 'coupon_restore test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for create_coupon
  # @param coupon_create 
  # @param [Hash] opts the optional parameters
  # @return [Coupon]
  describe 'create_coupon test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for delete_coupon
  # @param coupon_id 
  # @param [Hash] opts the optional parameters
  # @return [nil]
  describe 'delete_coupon test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for get_coupon
  # @param coupon_id 
  # @param [Hash] opts the optional parameters
  # @return [Coupon]
  describe 'get_coupon test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for list_coupons
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page 
  # @option opts [Integer] :page_size 
  # @option opts [Boolean] :is_active 
  # @option opts [String] :code 
  # @option opts [String] :discount_type 
  # @return [Array<Coupon>]
  describe 'list_coupons test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for update_coupon
  # @param coupon_id 
  # @param coupon_update 
  # @param [Hash] opts the optional parameters
  # @return [Coupon]
  describe 'update_coupon test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
