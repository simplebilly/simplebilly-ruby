...
module SimplebillyApi
  class OssDependency < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::OssDependency` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::OssDependency`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] dependency_type Value to be assigned
    def dependency_type=(dependency_type)
      if dependency_type.nil?
        fail ArgumentError, 'dependency_type cannot be nil'
      end
    ...
    def hash
      [dependency_type, license, name, version].hash
    end
