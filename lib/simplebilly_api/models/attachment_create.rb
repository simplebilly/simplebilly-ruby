...
module SimplebillyApi
  class AttachmentCreate < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::AttachmentCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::AttachmentCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pdfa_path')
        self.pdfa_path = attributes[:'pdfa_path']
      end
    ...
    def hash
      [contact_id, file_name, file_size, mime_type, original_name, pdfa_path, sha256_hash, uploaded_by].hash
    end
