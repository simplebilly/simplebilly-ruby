  ...
  class Employee < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::Employee` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::Employee`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [address, backup_employee_id, bic, city, country, created_at, date_of_birth, deleted_at, department_id, email, first_name, gender, hire_date, hourly_cost, iban, id, job_title, last_login, last_name, last_updated, monthly_salary, phone, state, status, tenant_id, updated_at, user_id, weekly_hours, zip].hash
    end
