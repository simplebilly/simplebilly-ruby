...
module SimplebillyApi
  class ProductionOrder < ApiModelBase
    # References the BOM entity.
    attr_accessor :bom_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::ProductionOrder` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::ProductionOrder`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'bom_id')
        self.bom_id = attributes[:'bom_id']
      end
    ...
    def hash
      [bom_id, components, end_date, notes, order_number, product_id, quantity, source_warehouse_id, start_date, status, target_warehouse_id].hash
    end
