...
module SimplebillyApi
  class QuotaOverview < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::QuotaOverview` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::QuotaOverview`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'metered')
        if (value = attributes[:'metered']).is_a?(Array)
          self.metered = value
        end
      else
        self.metered = nil
    ...
    def hash
      [features, is_trialing, limits, metered, plan, plan_name, trial_ends_at, usage].hash
    end
