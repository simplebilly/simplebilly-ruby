...
module SimplebillyApi
  ...
  class CreateTicketRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::CreateTicketRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::CreateTicketRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] message_body Value to be assigned
    def message_body=(message_body)
      if message_body.nil?
        fail ArgumentError, 'message_body cannot be nil'
      end
    ...
    def hash
      [channel_id, channel_type, customer_email, customer_id, customer_name, external_id, message_body, order_ref, subject].hash
    end
