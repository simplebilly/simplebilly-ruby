...
module SimplebillyApi
  class ReturnOrder < ApiModelBase
    # References the contact entity.
    attr_accessor :customer_contact_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::ReturnOrder` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::ReturnOrder`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'customer_contact_id')
        self.customer_contact_id = attributes[:'customer_contact_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          customer_contact_id == o.customer_contact_id &&
    ...
    def hash
      [customer_contact_id, customer_name, line_items, notes, order_id, order_number, return_number, return_reason, status, warehouse_id].hash
    end
