...
module SimplebillyApi
  class KontoReport < ApiModelBase
    attr_accessor :generated_at

    attr_accessor :konten
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::KontoReport` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::KontoReport`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'konten')
        if (value = attributes[:'konten']).is_a?(Array)
          self.konten = value
        end
      else
        self.konten = nil
    ...
    def list_invalid_properties
      ...
      if @konten.nil?
        invalid_properties.push('invalid value for "konten", konten cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] konten Value to be assigned
    def konten=(konten)
      if konten.nil?
        fail ArgumentError, 'konten cannot be nil'
      end
    ...
    def hash
      [generated_at, konten, period].hash
    end
