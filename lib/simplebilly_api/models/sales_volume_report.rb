...
module SimplebillyApi
  class SalesVolumeReport < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::SalesVolumeReport` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::SalesVolumeReport`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] grand_total Value to be assigned
    def grand_total=(grand_total)
      if grand_total.nil?
        fail ArgumentError, 'grand_total cannot be nil'
      end
    ...
    def hash
      [grand_total, items, total_count].hash
    end
