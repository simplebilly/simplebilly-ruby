...
module SimplebillyApi
  class ShippingThresholdUpdate < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::ShippingThresholdUpdate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::ShippingThresholdUpdate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reserve_stock')
        self.reserve_stock = attributes[:'reserve_stock']
      end
    ...
    def hash
      [is_active, max_sellable, name, notes, product_id, reserve_stock, warehouse_id].hash
    end
