...
module SimplebillyApi
  class PayrollCreatePayload < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::PayrollCreatePayload` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::PayrollCreatePayload`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'extra_payments')
        if (value = attributes[:'extra_payments']).is_a?(Array)
          self.extra_payments = value
        end
      end
    ...
    def hash
      [employee_ids, extra_payments, month, year].hash
    end
