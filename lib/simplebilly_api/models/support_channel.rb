...
module SimplebillyApi
  class SupportChannel < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::SupportChannel` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::SupportChannel`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] channel_type Value to be assigned
    def channel_type=(channel_type)
      if channel_type.nil?
        fail ArgumentError, 'channel_type cannot be nil'
      end
    ...
    def hash
      [channel_type, config, created_at, is_active, name, tenant_id, updated_at].hash
    end
