...
module SimplebillyApi
  class SepaDirectDebitResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::SepaDirectDebitResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::SepaDirectDebitResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] xml_content Value to be assigned
    def xml_content=(xml_content)
      if xml_content.nil?
        fail ArgumentError, 'xml_content cannot be nil'
      end
    ...
    def hash
      [content_type, filename, xml_content].hash
    end
