...
module SimplebillyApi
  class CouponValidation < ApiModelBase
    attr_accessor :code
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SimplebillyApi::CouponValidation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SimplebillyApi::CouponValidation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] discounted_amount Value to be assigned
    def discounted_amount=(discounted_amount)
      if discounted_amount.nil?
        fail ArgumentError, 'discounted_amount cannot be nil'
      end
    ...
    def hash
      [code, discount_type, discount_value, discounted_amount, max_discount_amount, reason, valid].hash
    end
